...
public class CacheControllerTest extends BaseTestController {
    ...
    public void getDictionaryList() throws Exception {
        // 列表查询
        ResultActions actions = mockMvc.perform(get("/cache/dictionary")
        ...
        // 列表查询
        ResultActions cacheActions = mockMvc.perform(get("/cache/dictionary")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk());
        String resultJson2 = cacheActions.andReturn().getResponse().getContentAsString();
        HashMap result2 = JSONObject.parseObject(resultJson2, HashMap.class);
        Assert.assertNotNull(result2);
        Assert.assertNotNull(result2.get(DEFAULT_DATA));
    }
